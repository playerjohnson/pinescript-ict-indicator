...
> 7. **The indicator now scores the Core Loop itself** (`ENABLE_SETUP`, group ㉘): a Dashboard "Setup" row shows the live per-side score out of 5 (killzone + bias + fresh sweep + fresh SMT + structure trend), and `A+ Long Setup` / `A+ Short Setup` alertconditions + `SETUP LONG/SHORT` webhooks fire once per triggering sweep when the score reaches `setup_minScore` and a live same-direction FVG exists on the retrace side (the webhook carries its C.E. — your entry price). On the fire bar a ▲/▼ "A+" marker also prints on-chart (`setup_showMarks`) — a chart-level signal that works without any alerts/webhook plan (Basic-tier accounts). Section 1's manual assembly remains the definition of what the score measures; the "the trader is the confluence engine" framing below predates this.
>
