...
### 🟠 M1 (GATE) — The whole technique rests on unverified TradingView behavior
The premise is that the crosshair **magnet snaps to an indicator's plotted series**. This cannot be confirmed statically (or from this environment). **Before anything else, confirm on a chart that the crosshair actually snaps to these plotted HTF O/H/L/C levels.** If it doesn't, the 24 plots add cost and clutter for zero benefit and should be reverted.
...
### 🟡 M3 — Price-axis / status-line / Data-Window clutter
`plot()` defaults to `display = display.all`, which includes `display.price_scale` (the right-axis last-value tag), `display.status_line`, and `display.data_window`. With 24 plots you get **24 price-axis tags, 24 status-line values, and 24 Data Window rows**, plus 24 toggleable entries in the Style tab.
**Try** `display = display.pane` on each to remove the clutter — **but do not assume this is the fix.** `display.pane` may stop the magnet from snapping (it depends on M1's mechanism). Test M1 with and without it: if magnet still snaps with `display.pane`, use it; if not, the clutter may be the unavoidable cost of the technique, and you'd hide the plots via the Style tab instead. Confidence: high on the clutter; the *remedy* is unverified.
