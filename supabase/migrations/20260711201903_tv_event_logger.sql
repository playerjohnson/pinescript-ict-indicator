...
-- dir: BSL sweep = bearish reversal expected (-1), SSL = bullish (+1); 0 = non-directional event.
...
select
  ...
  case when e.event like '%BSL%' then -1 when e.event like '%SSL%' then 1 else 0 end as dir,
...
-- Starter per-bucket stats at the 12-bar horizon (win = signed move in the reversal direction > 0).
